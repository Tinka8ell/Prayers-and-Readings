...
<body>'''
   ...
   def saveDaily(self, directory=None):
      ...
      title = "Daily prayer readings for: " + self.date.strftime("%A %d %B %Y")
   ...
   def html(self, f, showdivs=True):
      ...
      for paraType, para in self.meditation:
         ...
         if paraType == "H":
            ...
            prefix = "<p>"
            for p in para.split("\n"):
               print(prefix + cleanText(p).encode('ascii', 'xmlcharrefreplace').decode(), file=f)
               prefix = "<br/>"
            print("</p>", file=f)
      if showdivs:
         print("<hr/>", file=f)
...
if __name__ == "__main__":
   day = DailyPrayer("https://www.northumbriacommunity.org/offices/morning-prayer/", version="NLT")
   '''
   day.show()
   day.showDaily()
   print()
