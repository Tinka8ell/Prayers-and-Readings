...
<body>'''
            ...
            if (not ("p1" in pClass)) and len(strings) == 1 :
               ...
               if len(watchwords) > 0 or len(daily) > 0 or len(oneYear) > 0:
                  ...
                  if prayer:
                     ...
                     if passage:
                        # if we found a passage it was a reading
                        passage = passage.group(1).strip() # just take the passage, and not the optional version
   ...
   def saveDaily(self, directory=None):
      ...
      title = "Moravian daily readings for: " + self.date.strftime("%A %d %B %Y")
      with open(filename, 'w', encoding="utf-8") as f:
         ...
         self.html(f)
         print('<p><a href="year.html">' + "Today's bible in a year readings</a></p>", file=f)
         print("<hr/>", file=f)
      ...
      with open(filename, 'w', encoding="utf-8") as f:
         ...
         for group in self.oneYear:
            ...
            for passage in group[1]:
               ...
               bible.htmlParas(f)
               print("<hr/>", file=f)
         print('<p>' + tagText("Today's daily readings", 'a href="/day.html"') + "</p>", file=f)
         print("<hr/>", file=f)
         print(self.p3, file=f)
      if os.name == 'posix':
         # point symbolic link to file created:
...
'''
      with open(filename, 'w', encoding="utf-8") as f:
         ...
         for name in filelist:
            ...
            print("   <item>", file=f)
            print("      <title>Reading for " + pubDate + "</title>", file=f)
            print("      <link>" + self.prefix + source.replace(directory, "")[1:] + "/" + name + "</link>", file=f)
            print("      <description>Reading for " + pubDate + "</description>", file=f)
